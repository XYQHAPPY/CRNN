...
class DenseNet(nn.Module):
    def __init__(self, block, num_block, growth_rate=12, reduction=0.5, num_classes=300):
        super(DenseNet, self).__init__()
        self.growth_rate = growth_rate

        num_planes = 2 * growth_rate
        self.conv1 = nn.Conv2d(3, num_planes, kernel_size=3, padding=1, bias=False)
        #self.conv1 = nn.Conv2d(230, num_planes, kernel_size=3, padding=1, bias=False)
        ...
        self.linear_cls = nn.Linear(num_planes, num_classes)
        del num_planes,out_planes
    ...
    def forward(self, x):
        ...
        x = self.dense4(x)
        x = func.avg_pool2d(func.relu(self.bn(x)), 8)
        feature = x.view(x.size(0), -1)
        wordvec = self.linear_cls(feature)
        #return x,wordvec
        return feature,wordvec
